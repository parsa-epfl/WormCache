...
pub struct SerialSharedCache<const SET: usize, const WAY: usize, const EXCLUSIVE: bool> {
...
impl<const SET: usize, const WAY: usize, const EXCLUSIVE: bool> SerialSharedCache<SET, WAY, EXCLUSIVE> {
    pub fn new() -> Self {
        Self {
            blocks: crate::util::init_heap_array(|_| {
                std::array::from_fn(|_| SharedCacheBlock {
                    valid: false,
                    tag: 0,
                    ts: 0,
                })
    ...
    pub fn invalidate(&mut self, block_id: u64) -> bool {
        let set_id = (block_id % SET as u64) as usize;
        let mut blocks = &mut self.blocks[set_id];
...
impl<const SET: usize, const WAY: usize> SerialSharedCache<SET, WAY, true> {
    pub fn lookup(&mut self, block_id: u64) -> bool {
        let set_id = (block_id % SET as u64) as usize;
        let mut blocks = &mut self.blocks[set_id];
    ...
    pub fn evict_to(&mut self, block_id: u64, ts: u64) {
        let set_id = (block_id % SET as u64) as usize;
        let mut blocks = &mut self.blocks[set_id];
...
impl<const SET: usize, const WAY: usize> SerialSharedCache<SET, WAY, false> {
    pub fn lookup(&mut self, block_id: u64) -> bool {
        let set_id = (block_id % SET as u64) as usize;
        let mut blocks = &mut self.blocks[set_id];
