...
impl MH {
    fn where_is_the_block(&mut self, block_id: u64) -> BlockPosition {
        ...
        if self.shared_cache.lookup(0, block_id, 0, false).is_some() {
...
fn reversed_timestamp_from_the_same_core() {
    ...
    for l in 0..parameter::UNIFIED_PRI_CACHE_ASSO {
        ...
        mh.access_memory_pblock_id(0, block_id, l as u64 + ts + 1, CacheAccessType::DataRead);
    }
    ...
    assert_eq!(
        mh.access_memory_pblock_id(0, eval_block_id, 1, CacheAccessType::DataRead),
...
fn write_invalidation_coherence() {
    ...
    assert_eq!(
        mh.access_memory_pblock_id(0, block_id, 1, CacheAccessType::DataRead),
        CacheHierarchyAccessResult::Miss
    );
    // Core 1 get a read permission at 10.
    assert_eq!(
        mh.access_memory_pblock_id(1, block_id, 10, CacheAccessType::DataRead),
        CacheHierarchyAccessResult::HitInOtherPrivateCache
    );
    // Core 2 get a write permission at 50.
    assert_eq!(
        mh.access_memory_pblock_id(2, block_id, 50, CacheAccessType::DataWrite),
        CacheHierarchyAccessResult::HitInOtherPrivateCache
    );
...
fn rarw() {
    ...
    assert_eq!(
        mh.access_memory_pblock_id(0, block_id, 10, CacheAccessType::DataRead),
        CacheHierarchyAccessResult::Miss
    );
    // Core 0 get a write permission at 20.
    assert_eq!(
        mh.access_memory_pblock_id(0, block_id, 20, CacheAccessType::DataWrite),
    ...
    assert_eq!(
        mh.access_memory_pblock_id(1, block_id, 1, CacheAccessType::DataRead),
...
fn waw() {
    ...
    assert_eq!(
        mh.access_memory_pblock_id(0, block_id, 10, CacheAccessType::DataWrite),
        CacheHierarchyAccessResult::Miss
    );
...
fn rae() {
    ...
    for i in 0..parameter::UNIFIED_PRI_CACHE_ASSO {
        ...
        mh.access_memory_pblock_id(0, block_id, (100 + i) as u64, CacheAccessType::DataRead);
    }
...
fn eae() {
    ...
    let block_id = 1024;
    assert_eq!(
        mh.access_memory_pblock_id(0, block_id, 200, CacheAccessType::DataWrite),
        CacheHierarchyAccessResult::Miss
    );

    for i in 0..parameter::UNIFIED_PRI_CACHE_ASSO {
        let block_id: u64 = (100 * (i + 1) * PCACHE_SET + block_id as usize) as u64;
        mh.access_memory_pblock_id(0, block_id, (200 + i) as u64, CacheAccessType::DataRead);
    }
    ...
    for i in 0..parameter::UNIFIED_PRI_CACHE_ASSO {
        ...
        mh.access_memory_pblock_id(1, block_id, (10 + i) as u64, CacheAccessType::DataRead);
    }
